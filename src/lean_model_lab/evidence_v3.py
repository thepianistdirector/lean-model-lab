"""Registered prompt-intervention evidence with explicit task-quality and total-cost gates.
...
def _validate_inputs(workload: Any, config: Any, config_sha256: str,
    ...
    try:
        validate_workload_v3(workload)
        validate_config_v2(config)
    except (ContractError, KeyError, TypeError) as exc:
        raise EvidenceError(f'invalid frozen v3 inputs: {exc}') from exc
...
def _validate_attempt(attempt: Any, workload: dict, config: dict,
    ...
    _integer(attempt['schema_version'], 'schema_version', 3, 3)
...
def validate_attempt_v3(attempt: Any, workload: Any, *, config: Any,
                        config_sha256: str, workload_sha256: str) -> None:
...
def _summarize(attempt: dict, workload: dict, config: dict) -> dict:
    ...
    if workload['generator']['family'] in ('record-mixed-v1','record-mixed-explicit-v2'):
        families={key:source['prompt'].split('\n')[1].removeprefix('TASK ') for key,source in expected.items()}
        result['task_families']={}
        for family in sorted(set(families.values())):
            ids={key for key,value in families.items() if value==family}
            present=[r for r in records if r['request_id'] in ids]
            successes=sum(r['request_id'] in ids for r in correct_rows)
            result['task_families'][family]={'expected_count':len(ids),'observed_count':len(present),
                'quality_correct':successes,'quality_accuracy':successes/len(ids),
                'client_end_to_end_ns':_distribution([r['completed_ns']-r['arrival_ns'] for r in present]),
                'scope':'Declared constituent subgroup of one mixed population; no independent subgroup significance or 128-request-per-family claim.'}
    return result


def summarize_attempt_v3(attempt: Any, workload: Any, *, config: Any,
                         config_sha256: str, workload_sha256: str) -> dict:
    validate_attempt_v3(attempt, workload, config=config, config_sha256=config_sha256,
                         workload_sha256=workload_sha256)
    return _summarize(attempt, workload, config)


def evaluate_campaign_v3(attempts: Any, workload: Any, *, config: Any,
                         config_sha256: str, workload_sha256: str) -> dict:
    ...
    for attempt in ordered:
        ...
        for request in attempt['requests']:
            prompt_counts.setdefault((attempt['arm'], request['request_id']), set()).add(request['prompt_tokens'])
    drift_ids = {identifier for (arm, identifier), counts in prompt_counts.items() if len(counts) != 1}
    ...
    for concurrency in evaluation['concurrency_modes']:
        ...
        for pair in range(pair_count):
            ...
            if evaluation['output_token_parity'] and (not parity or prompt_mismatches):
                reasons.append(f'concurrency_{concurrency}_pair_{pair}_token_identity_or_parity_failed')
            base_quality = summary_by_id[baseline['attempt_id']]['quality_accuracy']
            candidate_quality = summary_by_id[candidate['attempt_id']]['quality_accuracy']
            if candidate_quality < base_quality:
                reasons.append(f'concurrency_{concurrency}_pair_{pair}_task_quality_regression')
            pairs.append({'quality_difference_candidate_minus_baseline': candidate_quality - base_quality,
                          'full_wall_efficiency_candidate_over_baseline': None, 'pair_index': pair, 'baseline_attempt_id': baseline['attempt_id'],
    ...
    for group in groups.values():
        ...
        for pair in group['pairs']:
            ...
            pair['eligible'] = True
            pair['full_wall_efficiency_candidate_over_baseline'] = base['full_wall_ns'] / cand['full_wall_ns']
        ...
        full_ratios = [pair['full_wall_efficiency_candidate_over_baseline'] for pair in group['pairs']]
        finding = ('TAIL_REGRESSION' if any(v > 1 + evaluation['maximum_tail_regression'] for v in tails) else
                   'PRACTICAL_GAIN_IN_ALL_PAIRS' if min(full_ratios) >= 1 + evaluation['practical_throughput_gain'] else
                   'NO_DEMONSTRATED_PRACTICAL_GAIN')
        group.update(full_wall_efficiency_ratio_median=statistics.median(full_ratios),
                     full_wall_efficiency_ratio_range=[min(full_ratios), max(full_ratios)], eligible=True, finding=finding, pair_count=len(ratios),
                     throughput_ratio_median=statistics.median(ratios), throughput_ratio_range=[min(ratios), max(ratios)])
    evidence_class = config['evidence_class']
    return {
        'schema_version': 3, 'evaluator_version': 'v3-prompt-task-quality-1', 'evidence_class': evidence_class,
        'config_sha256': config_sha256, 'workload_sha256': workload_sha256,
        'recipe_context': {'mechanism_id': config['recipe']['mechanism_id'],
            'research_protocol': config['recipe']['research_protocol'], 'recipe_sha256':digest(config['recipe']), 'purpose':config['recipe']['purpose'],
        ...
        'limitations': [
            ...
            'Generated structured-record tasks do not establish general language competence; frozen splits and separate code are workflow separation, not malicious-writer isolation.',
            'Changed-input comparisons require both arms to pass 95% exact task quality with no observed pairwise quality loss; output/token differences remain visible. A syntactic dependency certificate cannot guarantee model output.',
            'Primary practical-gain classification uses full attempt wall including input transformation/tokenization, hashing, startup and shutdown; service-only throughput is descriptive. Shared setup/search allocation cost is added by the inventory report.',
