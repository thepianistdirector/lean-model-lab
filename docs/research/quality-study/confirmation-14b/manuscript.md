...
## Queue-provenance packaging correction

The first integrated candidate03 ZIP omitted the exact original queue receipt, registration and product log despite retaining their inventory hashes. The [queue-provenance supplement](queue-provenance/README.md) now includes those unchanged bytes and recomputes the reported 90.012337226-second wait and 2774.945479864-second archive-process interval. The original candidate is retained. This corrects cost-envelope reproducibility without changing native observations, analysis values or scientific conclusions.
