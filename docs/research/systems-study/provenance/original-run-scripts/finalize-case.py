...
case_id=sys.argv[1];assert case_id in {'h1-small-reuse24','h2-small-reuse64','h3-large-reuse24'}
root=pathlib.Path.cwd();d=root/'.cache/v1-investigations-20260908/systems'/case_id;archive=root/'.cache/packaged-v1-platform-20260908-04/lean-model-lab.pyz';sha=hashlib.sha256(archive.read_bytes()).hexdigest();assert sha=='5f1aabf84635e33dd166ee656d5bcb5830f25a5f4f5d68433e2fd3e4498fb986'
...
run('export',[sys.executable,'tools/export_study.py',d/'study','--output',d/'export','--model-path',root/('.cache/prepared-14b-20260908-01/qwen2.5-14b-instruct-fp16-00001-of-00008.gguf' if case_id=='h3-large-reuse24' else '.cache/prepared-20260908-01/qwen2.5-0.5b-instruct-fp16.gguf')])

manifest_path=d.parent/'analysis-manifest.json';manifest=json.loads(manifest_path.read_text())
assert case_id not in [c['id'] for c in manifest['cases']], 'Case already analyzed'
manifest['cases'].append({'id':case_id,'directory':str(d.relative_to(root))})
