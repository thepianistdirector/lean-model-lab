...
def main():
 ap=argparse.ArgumentParser();ap.add_argument('--study',action='append',type=pathlib.Path,required=True);ap.add_argument('--output',type=pathlib.Path,required=True);args=ap.parse_args();args.output.mkdir(parents=True,exist_ok=True)
 ...
 for study in args.study:
  workload=read(study/'workload.json');config=read(study/'config.json');recipe=config['recipe'];mechanism=recipe['mechanism_id'];phase=recipe['purpose'];report_path=study.with_name(study.name.replace('-study','-report')+'.json');report=read(report_path)
  workload_rows[study.name]=workload['requests'];source={r['request_id']:r for r in workload['requests']};parsed={rid:parse(r['prompt']) for rid,r in source.items()};tables[study.name]={rid:r['group_id'] for rid,r in parsed.items()}
  ...
  for pi,arms in sorted(by_pair.items()):
   ...
   for rid in source:
    ...
    row={'study':study.name,'phase':phase,'mechanism':mechanism,'pair_index':pi,'request_id':rid,'family':parsed[rid]['family'],'full_correct':b['correct'],'candidate_correct':c['correct'],'outcome':outcome,'oracle_correct':b['correct'] or c['correct'],'features':parsed[rid]['features']};local_pairs.append(row);paired.append({k:v for k,v in row.items() if k!='features'})
  all_pair_rows[study.name]=local_pairs
  ...
  for rid in source:
   ...
   stability.append({'study':study.name,'request_id':rid,'family':parsed[rid]['family'],'full_correct_sequence':''.join('1' if r['correct'] else '0' for r in arm_rows['baseline']),'candidate_correct_sequence':''.join('1' if r['correct'] else '0' for r in arm_rows['candidate']),'full_unique_outputs':len({r['output'] for r in arm_rows['baseline']}),'candidate_unique_outputs':len({r['output'] for r in arm_rows['candidate']}),'per_pair_union_sequence':''.join('1' if by_pair[pi]['baseline'][rid]['correct'] or by_pair[pi]['candidate'][rid]['correct'] else '0' for pi in sorted(by_pair)),'conservative_oracle_correct':full or candidate})
  ...
  queue=read(pathlib.Path(str(study)+'.queue')/'receipt.json');costs.append({'study':study.name,'native_offered':len(source)*len(ra),'unique_tables':len(set(tables[study.name].values())),'queue_execution_s':(queue['finished_ns']-queue['execution_started_ns'])/1e9,'attempt_full_wall_s':report['accounting']['full_wall_ns']/1e9,'enclosing_study_span_s':report['accounting']['elapsed_span_ns']/1e9,'original_allocation_full_wall_ns':report['accounting']['full_cost_accounting'].get('registered_full_wall_ns'),'scope':'Queue, attempt and allocation scopes overlap; never add them. Only disjoint queue executions are summed across studies.'})
  inventory.append({'study':str(study),'config_sha256':sha(study/'config.json'),'workload_sha256':sha(study/'workload.json'),'report_sha256':sha(report_path),'queue_sha256':sha(pathlib.Path(str(study)+'.queue')/'receipt.json'),'native_attempts':len(ra),'offered_each':len(source),'families':dict(collections.Counter(r['family'] for r in parsed.values()))})
  checks.append({'study':study.name,'status':'PASS','checks':['All offered native outputs scored against independent record interpreter','Independent correct counts equal packaged report','All relevant candidate prompts preserve symbolic answers','All four paired outcome classes retained','Per-pair and conservative oracle denominators kept separate']})
 fit='negative-fit-study04';cal='negative-calibration-study04';control='negative-budget-control-study04';confirm=[name for name in tables if name.startswith('negative-confirm-')]
