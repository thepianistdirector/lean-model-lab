...
for spec in manifest.get('closed_figures',[]):
 ...
 for arm,label,color,offset in [('baseline','Original full v1','#4c6585',-.18),('candidate',spec['candidate_label'],'#b86939',.18)]:
  ...
  for i,key in enumerate(['accuracy','full_wall_s']):
   bars=axs[i].bar([j+offset for j in range(2)],[float(r[key])*(100 if i==0 else 1) for r in selected],width=.34,label=label,color=color);axs[i].bar_label(bars,labels=[r['correct']+'/'+r['offered'] if i==0 else f"{float(r[key]):.1f}" for r in selected],padding=3,fontsize=8)
 axs[0].axhline(95,color='#a12828',linestyle='--',label='95% required');axs[0].set_ylim(0,110);axs[0].set_ylabel('Exact task accuracy (%)');axs[0].legend(frameon=False,fontsize=8,loc='upper left',bbox_to_anchor=(0,.82));axs[0].set_title('Closed development advance branch')
