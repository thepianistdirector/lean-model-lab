...
def verify(args):
    ...
    if not source['inventory_complete'] or len(source['attempts'])!=12 or any(a['status']!='COMPLETED' for a in source['attempts']):
        raise ValueError('parent comparison must retain all twelve completed arm runs before the recovery control')
    records=list(source['setup_accounting']['records'])
    for a in source['attempts']:
        records.append({'phase':'predecessor-comparison','status':'COMPLETED','started_ns':a['started_ns'],
            'finished_ns':a['finished_ns'],'bytes_acquired':0,'details':json.dumps({
                'source_study':args.parent_study.name,'attempt_id':a['attempt_id'],'attempt_sha256':digest(a),
                'observed_requests':len(a['requests']),'scope':'retained completed main comparison; no duplicated measurement'},sort_keys=True)})
    records.sort(key=lambda r:r['started_ns']);allocation_start=records[0]['started_ns']
    deadline=allocation_start+LIMITS['full_wall_seconds']*10**9
    if deadline-started<300*10**9:raise ValueError('less than five minutes remain in original allocation; no recovery run started')
    prepared=read_json(args.setup)
    setup=root/'setup-with-parent-costs.json';write_json_new(setup,{**prepared,'records':records})
    logs=[];children=[]
    ...
    try:
        p=launch('initial-run','run','--server',args.server,'--model',args.model,'--setup',setup,
                 '--admission',args.admission,'--output',study)
        checkpoints=wait_until(p,lambda:[e for e in events() if e['kind']=='checkpoint']
                               if sum(e['kind']=='checkpoint' for e in events())>=5 else None,seconds=120)
        ...
        p=launch('resume-run','resume',study,'--server',args.server,'--model',args.model,'--admission',args.admission)
        def restarted_terminal():
            ...
            return None
        replacement=wait_until(p,restarted_terminal,seconds=360)
        if replacement['arm']!='baseline' or replacement['concurrency']!=1 or replacement['pair_index']!=0 or len(replacement['requests'])!=128:
        ...
        result={'schema_version':1,'status':'PASS','evidence_class':'MEASURED_RECOVERY_CONTROL',
            ...
            'archive_sha256':file_digest(archive),'original_attempt_id':original_id,'replacement_attempt_id':replacement['attempt_id'],
            'observed_requests_before_crash':len(first['requests']),'replacement_requests':len(replacement['requests']),
...
if __name__=='__main__':
    ...
    for name in ('parent-study','archive','server','model','setup','admission','output'):
